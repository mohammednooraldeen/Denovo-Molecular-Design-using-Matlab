function mol_updated = mutation_mol( mol )
...
for i=1:iterations;
%choices=mol_updated((find(mol_updated(:,5)>=2)),:)
%s_choices=size(choices);s_choices=s_choices(1,1);

    
%selection and removing single atom
%s_mol_updated=size(mol_updated),s_mol_updated=s_mol_updated(1,1);
choices_updated=mol_updated((find(mol_updated(:,5)>=2)),:)
...
%choices_updated= choices_updated(~ismember(choices_updated(:,1),choices_updated(choices_updated(find(choices_updated(:,5)>=2),6),1),'rows'),:)
